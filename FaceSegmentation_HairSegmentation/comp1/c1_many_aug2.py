...
img_size = 256  # image size that will be used. need to be a multiple of 2^4, since we have 4 MaxPooling2D layers
...
hair_path_target = base_path + "training_masks_hair/training_masks_hair"  # path of hair masks

train_path_valid = base_path + "validation_images/validation_images"  # path of validation images
hair_path_valid = base_path + "validation_masks_hair/validation_masks_hair"  # path of validation masks
...
class CustomModelCheckpoint(callbacks.ModelCheckpoint):
    ...
    def on_epoch_end(self, epoch, logs=None):
        cur_epoch = epoch + 1
        # Only save epoch if it is epoch 3 or greater
        if (cur_epoch >= 3):
            ...
            for i in list1[4:]:  # keep the best 4 weights, which will have the lowest val_loss
                print("\nRemoving {}".format(i))  # printing the removal of a weight
...
train_data, train_target = load_images_image_aug(train_path, "train_img", hair_path_target, "train_mask")  # load in images and mask + their augmentations
...
train_valid, train_valid_target = load_images_image_aug(train_path_valid, "validation_img",
                                                        hair_path_valid, "validation_mask")
